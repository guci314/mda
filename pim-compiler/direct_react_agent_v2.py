#!/usr/bin/env python3
"""改进版ReactAgentGenerator，修复测试运行机制"""
    ...
    def generate_code(self, psm_content: str, output_dir: str):
        ...
        def run_tests(test_dir: Optional[str] = "tests", verbose: bool = True) -> str:
            ...
            try:
                ...
                # 构建命令 - 使用当前Python解释器运行pytest
                cmd = [python_exe, "-m", "pytest"]
                if verbose:
                    cmd.extend(["-v", "-s"])
                cmd.extend([str(test_path), "--tb=short", "-x"])  # -x: 第一个失败就停止
                ...
                output += f"Test directory: {test_path}\n"
...
工作流程：
1. 生成所有代码文件
2. 使用 install_dependencies 工具安装项目依赖
3. 使用 run_tests 工具运行测试
4. 如果测试失败，分析错误并修复代码
5. 重复步骤3-4直到所有测试通过

注意：
- 必须先安装依赖，然后才能运行测试
- 测试可能因为导入错误失败，这通常意味着需要先安装依赖
- 如果遇到 "No module named pytest" 错误，说明需要先运行 install_dependencies"""
...
重要：生成代码后，你必须：
1. 使用 install_dependencies 工具安装项目依赖
...
def main():
    ...
    output_dir = Path("output/react_agent_v2")
    
    logger.info(f"Using generator: react-agent (v2)")
    ...
    try:
        ...
        logger.info("Initialized ReactAgentGenerator v2 with improved test mechanism")
        ...
        # 步骤2：生成代码
        logger.info("Step 2: Generating code with test execution...")
        ...
        print("=" * 50)
        print(f"Generator: react-agent v2")
