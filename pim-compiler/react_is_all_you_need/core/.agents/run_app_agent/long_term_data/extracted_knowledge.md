...
- 执行“重启”或“确保运行”类任务的标准流程是：1. 检查服务是否已在运行（如 `lsof -t -i:<port>`）。2. 如果在运行，则停止它（如 `kill <PID>`）。3. 启动服务。4. 验证服务已成功启动（如 `curl` 探测端点）。
...
- 任务完成后，应明确声明任务完成并停止执行，避免对已验证成功的条件进行不必要的重复检查。例如，在成功完成“重启”任务后，不应再次执行停止和启动操作。
- 处理多步骤指令时（如“1. 如果A，则B；2. 执行C”），应将其理解为一个连贯的计划，而非独立的命令序列。前一步骤的结果会影响后续步骤的执行，最终目标是达成整个计划的最终状态。
- **(新)** 当任务成功条件明确指定为端口状态时（如"8000端口没有进程"），仅需验证端口状态即可确认任务完成，无需额外检查。
...
- **操作偏好**: 倾向于保持环境整洁，会主动清理任务执行过程中产生的临时或冗余进程。应避免在任务逻辑中陷入不必要的循环或重复执行已完成的步骤。
- **交互模式**: 用户倾向于使用高层次、目标驱动的指令，其中可能包含条件逻辑（如“如果应用正在运行，则停止它”）。助手必须能够将这些步骤解析为一个单一的、连贯的计划，并按逻辑顺序执行，而不是作为独立的命令列表。
- **任务完成标准**: 用户可能会在指令中提供明确的“终止条件”或“成功标准”，助手必须严格遵守这些标准来判断任务是否完成，并在完成后明确声明并停止执行。
```