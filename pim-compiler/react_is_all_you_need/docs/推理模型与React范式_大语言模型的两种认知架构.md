...
1. Yao, S., Zhao, J., Yu, D., 等. (2022). ReAct: Synergizing Reasoning and Acting in Language Models. *arXiv 预印本 arXiv:2210.03629*.

2. Wei, J., Wang, X., Schuurmans, D., 等. (2022). Chain-of-Thought Prompting Elicits Reasoning in Large Language Models. *神经信息处理系统进展*.

3. Brown, T., Mann, B., Ryder, N., 等. (2020). Language Models are Few-Shot Learners. *神经信息处理系统进展*.

4. van der Aalst, W. (2016). Process Mining: Data Science in Action. *Springer*.

5. Ouyang, L., Wu, J., Jiang, X., 等. (2022). Training language models to follow instructions with human feedback. *神经信息处理系统进展*.
...
- **对话式 LLM**：GPT-4、Claude-3、Gemini-1.5
- **推理模型**：DeepSeek-Reasoner、OpenAI o1-preview
- **任务**：代码开发的多智能体协调
- **指标**：任务完成、状态一致性、重试处理
...
代码生成器创建 MathUtils。
测试器验证功能。
如果测试失败，生成器修复问题（最多 3 次）。
最后，审查员为代码评分。
```

---

*论文准备提交至：大语言模型认知架构研讨会*

*通讯作者：[PIM 编译器项目研究团队]*

## 作者说明

本文基于实际工程实践中的发现，通过对比实验揭示了 LLM 认知架构的深层规律。这一发现源于一个看似简单的问题：为什么 BPMN 工作流在普通 LLM 上失败，却在推理模型上成功？答案揭示了 AI 系统设计的一个基本原则：**认知架构与工作流表示必须匹配**。更深层的洞察是：这两种架构分别体现了西方哲学中的两大认识论传统——经验主义和理性主义。

这不仅是技术发现，更是对 AI 系统本质的理解：不同的训练方法创造了不同的"思维方式"，而这些思维方式决定了它们如何最好地与人类协作。

## 哲学意义

将 React 范式命名为"经验主义"，将 Plan-Execute 范式命名为"理性主义"，不仅仅是术语上的改变，而是揭示了 AI 发展正在重演人类认识论的古老辩论：

- **经验主义传统**（培根、洛克、休谟）：知识来自经验和观察
- **理性主义传统**（笛卡尔、斯宾诺莎、莱布尼茨）：知识来自理性和推理

LLM 的两种架构恰好体现了这两种认知方式，这或许暗示着：**人工智能的发展不是要超越人类认知，而是要完整地实现人类认知的所有维度**。