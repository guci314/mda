...
**❌ 绝对错误的做法**（像DeepSeek那样浪费17轮）：
```python
# ❌ 逐块读取，浪费大量轮次
read_file(file_path="order_tool.py", offset=100, limit=200) # 第1轮
read_file(file_path="order_tool.py", offset=300, limit=200) # 第2轮
read_file(file_path="order_tool.py", offset=500, limit=200) # 第3轮
# ...需要10+轮才能读完！
...
```python
# ✅ 最佳：直接读取整个文件（不指定参数）
read_file(file_path="order_tool.py")  # 一次读完整个文件！

# ✅ 代码文件通常都很小，直接读取
read_file(file_path="main.py")  # Python文件直接读取
read_file(file_path="config.json")  # JSON文件直接读取
```

**黄金规则**：
- **99%的代码文件**：直接`read_file(path)`，不要offset/limit
- **避免逐块读取**：这会浪费10+轮才能找到需要的内容
- **记住**：分块读取=低效，直接读取=高效
...
## 📝 经验管理
**简要说明**：使用@learning函数更新experience.md，使用@memory函数更新agent_knowledge.md。
详细指导请参考learning_functions.md和fractal_agent_knowledge.md。

### 🎯 知识体系概要
- **DNA知识**：默认加载的4个核心文件（详见agent_essence.md）
- **个体知识**：agent_knowledge.md和experience.md（详见fractal_agent_knowledge.md）
