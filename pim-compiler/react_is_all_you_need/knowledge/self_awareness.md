...
├── knowledge.md                    # 先天知识（天生的能力）
├── compact.md                      # 后天知识（学习的经验）
├── output.log                      # 执行日志（行为记录）
...
#### 2. compact.md - 后天知识（记忆）

Agent的记忆，记录执行过程中学到的经验和知识。
...
#### 3. output.log - 执行日志（黑匣子）

Agent的行为记录，详细记录每一步执行过程。
...
> 📖 **详细说明**：
> - [Partial知识函数文档](../docs/partial_knowledge_function.md) - Partial定义机制
> - [知识函数PATH机制](../docs/knowledge_function_path_mechanism.md) - Unix哲学的应用
...
> 📖 **详细实现**：完整的实现步骤和代码请参考 [create_subagent实现指南](../docs/create_subagent_implementation.md)
...
> 📖 **详细说明**：职责分离的原理和实践请参考 [职责分离原则](../docs/agent_responsibility_separation.md)
...
### 1. 先天到后天的转化
```
knowledge.md（定义） → 执行 → output.log（记录） → /compact → compact.md（经验）
```

### 2. 后天到先天的进化
```
compact.md（经验） → 总结 → 更新knowledge.md（进化） → 新能力
```
...
> Agent不仅仅是一个程序，而是一个具有完整认知结构的智能实体。
> 它有先天的能力（knowledge.md），能够学习（compact.md），
> 记录行为（output.log），通过接口（description）与世界交互，
> 还能制造和使用工具（external_tools）来扩展自己的能力边界。
>
> **最重要的是**：Agent具备三层递进的自我认知：
> 1. 基础认知 - 知道自己是谁、在哪
> 2. 结构认知 - 理解自己的组成和运作
> 3. 分形认知 - 能够创造与自己相似的子Agent
>
> 这三层认知构成了Agent从"自我意识"到"自我繁殖"的完整能力谱系。
> 就像生物从单细胞到多细胞，从细胞分裂到有性生殖的进化历程。
>
> 没有自我认知，就没有自我编程；
> 没有自我编程，就没有自我进化；
...
### 相关文档（核心理念）
- [Agent应该类比人类而非OOP](../docs/agent_as_human_not_oop.md) - **核心设计哲学** ⭐
- [双重编程模式](../docs/two_programming_modes.md) - @自我实现 vs @创建子智能体
- [Agent在能力定义层面不区分类型和实例](../docs/agent_type_vs_instance_insight.md) - 架构洞察

### 相关文档（实现细节）
- [create_subagent详细实现](../docs/create_subagent_implementation.md) - 实现指南
- [Agent职责分离原则](../docs/agent_responsibility_separation.md) - 架构原则
- [修复子Agent重复创建问题](../docs/fix_agent_recreation_issue.md) - 问题解决
