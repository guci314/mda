...
Agent系统基于对象模型和Event Sourcing架构：
- **世界**是一个对象，拥有状态和方法
- **Agent**是方法的执行者，拥有知识和工作内存
- **Session**是方法执行的不可变日志

```
系统 = {
    世界: {  # 客观、共享
        状态: world_state.md,         # 根目录，全局共享
        历史: .sessions/*.md,         # 事件日志
    },
    Agent: {  # 主观、私有
        知识: .notes/{name}/agent_knowledge.md,  # 私有知识
        记忆: .notes/{name}/task_process.md,     # 工作记忆
    }
}
```
...
### 4. task_process.md（Agent的工作记忆）
**性质**：当前任务的执行计划和进度
**位置**：`{work_dir}/.notes/{agent_name}/`
**生命周期**：任务期间，任务结束后清空或归档
**核心特性**：
- **动态TODO**：必须反映真实决策，不是静态模板
- **状态外部化**：每个TODO完成后立即记录结果
- **路径选择**：决策后删除未选择的路径
- **任务完成前必须包含**：创建session记录的TODO项
...
def complete_task(task_result):
    ...
    # 3. 更新世界状态（强制）
    if not update("world_state.md", task_result):
