"""PSM to Code generator using DeepSeek"""
...
class PSMtoCodeDeepSeekGenerator:
    """使用 DeepSeek 将 PSM 转换为可执行代码"""
    ...
    def generate_sync(self, psm_file: Path, output_dir: Path) -> Dict[str, Path]:
        ...
        # 根据平台生成代码
        if platform == 'fastapi':
            return self._generate_fastapi_code_sync(psm_data, output_dir)
        elif platform == 'spring':
            raise NotImplementedError("Spring code generation is not yet implemented")
        else:
            raise ValueError(f"Unsupported platform: {platform}")
    ...
    async def _generate_fastapi_code(self, psm_data: Dict[str, Any], output_dir: Path) -> Dict[str, Path]:
        ...
        for filename, prompt_generator in components:
            ...
            # 生成消息
            messages = prompt_generator(psm_data)
            
            # 调用 DeepSeek
            response = await self.llm.ainvoke(messages)
            code = self._extract_code(str(response.content), filename)
    ...
    def _generate_fastapi_code_sync(self, psm_data: Dict[str, Any], output_dir: Path) -> Dict[str, Path]:
        """同步生成 FastAPI 代码"""
        ...
        # 生成配置文件（同步版本）
        self._generate_config_files_sync(psm_data, output_dir)
        
        return generated_files
    
    def _generate_models_prompt(self, psm_data: Dict[str, Any]) -> list:
        """生成 SQLAlchemy 模型的消息"""
        psm_yaml = yaml.dump(psm_data, allow_unicode=True)
        
        system_prompt = """你是一个 Python FastAPI 专家，擅长生成高质量的 SQLAlchemy 2.0 数据模型代码。
你需要根据 PSM（平台特定模型）生成完整的、可直接运行的代码。"""
        
        user_prompt = f"""根据以下 PSM（平台特定模型）生成 SQLAlchemy 2.0 数据模型代码。
...
只输出 Python 代码，不要解释。确保代码可以直接运行。"""
    ...
    def _generate_schemas_prompt(self, psm_data: Dict[str, Any]) -> list:
        """生成 Pydantic schemas 的消息"""
        psm_yaml = yaml.dump(psm_data, allow_unicode=True)
        
        system_prompt = """你是一个 Python FastAPI 专家，擅长生成 Pydantic v2 schemas。"""
        
        user_prompt = f"""根据以下 PSM 生成 Pydantic v2 schemas（请求/响应模型）。
...
只输出 Python 代码。"""
    ...
    def _generate_services_prompt(self, psm_data: Dict[str, Any]) -> list:
        """生成业务逻辑服务的消息"""
        psm_yaml = yaml.dump(psm_data, allow_unicode=True)
        
        system_prompt = """你是一个 Python FastAPI 专家，擅长生成业务逻辑服务层代码。"""
        
        # 使用普通字符串拼接，避免 f-string 解析问题
        user_prompt = f"""根据以下 PSM 生成业务逻辑服务层代码。
...
只输出完整的 Python 代码。"""
    ...
    def _generate_api_prompt(self, psm_data: Dict[str, Any]) -> list:
        """生成 API 路由的消息"""
        psm_yaml = yaml.dump(psm_data, allow_unicode=True)
        
        system_prompt = """你是一个 Python FastAPI 专家，擅长生成 RESTful API 路由代码。"""
        
        user_prompt = f"""根据以下 PSM 生成 FastAPI 路由代码。
...
只输出完整的 Python 代码。"""
    ...
    def _generate_main_prompt(self, psm_data: Dict[str, Any]) -> list:
        """生成主程序入口的消息"""
        system_prompt = """你是一个 Python FastAPI 专家，擅长生成应用程序主入口文件。"""
        
        description = psm_data.get('description', 'API Service')
        
        user_prompt = f"""生成 FastAPI 应用程序的主入口文件 main.py。
...
只输出 Python 代码。"""
    ...
    def _generate_database_prompt(self, psm_data: Dict[str, Any]) -> list:
        """生成数据库配置的消息"""
        system_prompt = """你是一个 Python FastAPI 专家，擅长生成数据库配置代码。"""
        
        user_prompt = """生成 FastAPI 的数据库配置文件 database.py。
...
只输出 Python 代码。"""
    ...
    def _generate_requirements_prompt(self, psm_data: Dict[str, Any]) -> list:
        """生成 requirements.txt 的消息"""
        system_prompt = """你是一个 Python FastAPI 专家，生成项目依赖列表。"""
        
        user_prompt = """生成 FastAPI 项目的 requirements.txt 文件。
...
"""
    ...
    def _generate_config_files_sync(self, psm_data: Dict[str, Any], output_dir: Path):
        """同步生成配置文件"""
        # .env 文件
        env_content = """# 数据库配置
DATABASE_URL=postgresql://postgres:postgres@localhost:5432/library_db
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python psm_to_code_deepseek.py <psm_file> <output_dir>")
        sys.exit(1)
    ...
    async def main():
        ...
        generator = PSMtoCodeDeepSeekGenerator()
        try:
