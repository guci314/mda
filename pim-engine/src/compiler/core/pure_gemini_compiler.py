...
"""
    ...
    def _execute_with_monitoring(self, prompt: str, work_dir: Path, env: dict, 
        ...
        if target_dir.exists():
            ...
            py_files = [f for f in final_files if f.suffix == ".py" and f.is_file()]
            
            # Log what was generated for debugging
            if file_count > 0:
                logger.info(f"Generated {file_count} files, including {len(py_files)} Python files")
                # Check if main.py exists
                main_py = any("main.py" in str(f) for f in final_files)
                if not main_py:
