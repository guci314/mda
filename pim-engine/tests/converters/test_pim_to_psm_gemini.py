"""Test PIM to PSM converter with real Gemini CLI"""
...
class TestPIMtoPSMGeminiConverter(unittest.TestCase):
    """测试 PIM to PSM 转换器"""
    
    @classmethod
    def setUpClass(cls):
        """设置测试环境"""
        cls.converter = PIMtoPSMGeminiConverter()
...
"""
    ...
    def test_platform_specific_conversion(self):
        """测试不同平台的转换"""
        pim_content = """# 用户系统
...
"""
        ...
        pim_file = self.test_dir / "user_system.md"
        pim_file.write_text(pim_content, encoding='utf-8')
        ...
        fastapi_psm = self.loop.run_until_complete(test_fastapi())
        
        self.assertEqual(fastapi_psm['platform'], 'fastapi')
        
        # 验证 FastAPI 特定的配置
        if 'configurations' in fastapi_psm:
            config = fastapi_psm['configurations']
            # FastAPI 应该使用 SQLAlchemy
            self.assertIn('orm', str(config).lower())
